...
int main() {
  ...
  for (int n = 1; n <= 10; ++n) {
    ...
    auto timeAll = std::chrono::duration<double, std::milli>(end - start).count();

    start = std::chrono::high_resolution_clock::now();
    auto p1 = getPerm1(tree, number);
    end = std::chrono::high_resolution_clock::now();
    auto timeOne = std::chrono::duration<double, std::milli>(end - start).count();

    start = std::chrono::high_resolution_clock::now();
    auto p2 = getPerm2(tree, num);
    end = std::chrono::high_resolution_clock::now();
    auto timeTwo = std::chrono::duration<double, std::milli>(end - start).count();

    fout << n << "," << timeAll << "," << timeOne << "," << timeTwo << "\n";
